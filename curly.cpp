...
CURLcode Curly::Fetch(std::string url){
	...
	if(curlErr == CURLE_OK){
		// assumming everything is ok, get the content type and status code
...
size_t Curly::HttpContent(void* ptr, size_t size, size_t nmemb, void* stream){
...
size_t Curly::HttpHeader(void* ptr, size_t size, size_t nmemb, void* stream){
	Curly* handle = (Curly*)stream;
	size_t data_size = size*nmemb;
	if(handle != NULL){
